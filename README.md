...
不用手动编辑 `config.toml`，所有服务器配置都能在飞书里完成。

发送 `/server` 会看到紧凑的服务器列表（每行一台），右侧有 `···` 菜单，包含：

| 操作 | 功能 |
|---|---|
| 🖥️ 选择 | 切换到该服务器（自动测试 SSH 连通性） |
| 🔧 测试连接 | 验证 SSH + claude CLI 是否可用 |
| ⚙️ 编辑 | 弹表单修改 name / host / user / key 路径 / 默认 cwd（留空字段保持不变） |
| 🗑 删除 | 二次确认后从配置中移除 |

**添加新服务器：** 点击列表底部 **➕ 添加服务器**，填表单（ID、名称、主机、用户名），提交后直接拖 `.pem` 文件上传 SSH 密钥，自动保存配置即可使用。

**替换现有密钥：** 在编辑表单里点 **📤 重新上传密钥**，再拖一个新 `.pem` 文件即可，原密钥文件会被覆盖。

> 所有改动都会原子写回 `config.toml`（tmp + rename），重启进程后依然生效。SSH 密钥保存在 `<data_dir>/keys/<server_id>.pem`，删除服务器时 PEM 文件不会被自动清理。
